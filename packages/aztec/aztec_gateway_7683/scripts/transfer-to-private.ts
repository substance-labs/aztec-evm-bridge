...
const main = async () => {
  const logger = createLogger("transfer-to-private")
  logger.info("Transferring tokens from public to private balance (shielding)...")
  ...
  logger.info(`Transferring ${amount} tokens from public to private...`)
  await token.methods
    .transfer_public_to_private(account.getAddress(), account.getAddress(), BigInt(amount), 0n)
    .send({
  ...
  logger.info(`✅ ${amount} tokens successfully transferred to private balance`)
